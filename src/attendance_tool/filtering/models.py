...
class FilterResult:
    ...
    def get_summary(self) -> Dict[str, Any]:
        """サマリー情報取得"""
        filtered_ratio = (
            self.filtered_count / self.original_count
...
class DateFilterConfig:
    ...
    def from_yaml(cls, yaml_path: str) -> "DateFilterConfig":
        ...
        try:
            ...
            return cls(
                default_date_column=date_filter_config.get(
                    "default_date_column", "work_date"
                ),
                auto_detect_columns=date_filter_config.get("auto_detect_columns", True),
                leap_year_adjustment=date_filter_config.get(
                    "leap_year_adjustment", True
                ),
                month_end_adjustment=date_filter_config.get(
                    "month_end_adjustment", True
                ),
                invalid_date_handling=date_filter_config.get(
                ...
                parallel_processing=date_filter_config.get(
