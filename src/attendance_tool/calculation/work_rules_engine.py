...
class WorkRulesEngine:
    ...
    def get_standard_work_minutes(self) -> int:
        ...
        return self.work_rules.get("working_hours", {}).get(
            "standard_daily_minutes", 480
        )

    def get_standard_start_time(self) -> time:
        """標準開始時刻を取得 - Green Phase最小実装"""
        start_time_str = self.work_rules.get("working_hours", {}).get(
    ...
    def get_standard_end_time(self) -> time:
        ...
        end_time_str = self.work_rules.get("working_hours", {}).get(
    ...
    def calculate_overtime_premium(
        self, overtime_minutes: int, work_date: date, start_time: time, end_time: time
    ) -> int:
    ...
    def get_early_leave_threshold_minutes(self) -> int:
        ...
        return self.work_rules.get("tardiness", {}).get(
            "early_leave_threshold_minutes", 1
        )

    def get_rounding_config(self) -> Dict[str, Any]:
        """時間丸め設定を取得 - Refactor Phase追加"""
        return self.work_rules.get("tardiness", {}).get(
            "rounding", {"unit_minutes": 15, "method": "up"}
        )
    ...
    def get_monthly_overtime_limit(self) -> int:
        ...
        return (
            self.work_rules.get("overtime", {})
            .get("limits", {})
            .get("monthly_overtime_limit", 2700)
        )

    def get_max_consecutive_work_days(self) -> int:
        """連続勤務日数上限を取得 - Refactor Phase追加"""
        return (
            self.work_rules.get("validation", {})
            .get("warnings", {})
            .get("consecutive_work_days", 6)
        )

    # TASK-202 Red Phase: 違反チェックメソッド追加

    def check_daily_work_hour_violations(
        self, record: "AttendanceRecord"
    ) -> List["WorkRuleViolation"]:
    ...
    def check_weekly_work_hour_violations(
        ...
        legal_weekly_limit = self.work_rules.get("work_hour_limits", {}).get(
            "weekly_legal_minutes", 2400
        )  # 40時間
        warning_weekly_limit = self.work_rules.get("work_hour_limits", {}).get(
            "weekly_warning_minutes", 3600
        )  # 60時間

        for week_start, week_records in weekly_groups.items():
            total_week_minutes = sum(
                record.get_work_duration_minutes() or 0 for record in week_records
            )
            ...
            if total_week_minutes > legal_weekly_limit:
                ...
                level = (
                    ...
                    if total_week_minutes > warning_weekly_limit
                    else ViolationLevel.WARNING
                )
                violation_type = (
                    "weekly_excessive_work"
                    if total_week_minutes > warning_weekly_limit
                    else "weekly_overtime"
                )
    ...
    def check_monthly_violations(
        ...
        special_limit = (
            self.work_rules.get("overtime", {})
            .get("limits", {})
            .get("special_monthly_limit", 6000)
        )
        ...
        if total_overtime_minutes > monthly_limit:
            ...
            work_records = [
                r
                for r in records
                if r.get_work_duration_minutes() and r.get_work_duration_minutes() > 0
            ]
            if work_records:
    ...
    def check_break_time_violations(
        self, record: "AttendanceRecord"
    ) -> List["WorkRuleViolation"]:
    ...
    def check_consecutive_work_violations(
        self, records: List["AttendanceRecord"]
    ) -> List["WorkRuleViolation"]:
        ...
        for record in sorted_records:
            ...
            is_work_day = record.work_status == "出勤" or (
                work_minutes is not None and work_minutes >= 240
            )  # 4時間以上
            ...
            if is_work_day:
                ...
                critical_days = self.work_rules.get("consecutive_work_limits", {}).get(
                    "critical_days", 10
                )
    ...
    def check_night_work_violations(
        self, record: "AttendanceRecord"
    ) -> List["WorkRuleViolation"]:
    ...
    def check_holiday_work_violations(
        self, record: "AttendanceRecord"
    ) -> List["WorkRuleViolation"]:
    ...
    def generate_compliance_report(
        ...
        critical_count = len(
            [v for v in violations if v.level == ViolationLevel.CRITICAL]
        )
        violation_count = len(
            [v for v in violations if v.level == ViolationLevel.VIOLATION]
        )
        warning_count = len(
            [v for v in violations if v.level == ViolationLevel.WARNING]
        )
    ...
    def get_daily_legal_minutes(self) -> int:
        ...
        return self.work_rules.get("work_hour_limits", {}).get(
    ...
    def get_night_work_start_time(self) -> time:
        """深夜労働開始時刻を取得"""
        time_str = self.work_rules.get("night_work_rules", {}).get(
    ...
    def get_consecutive_work_critical_days(self) -> int:
        """連続勤務重大違反日数を取得"""
        return self.work_rules.get("consecutive_work_limits", {}).get(
            "critical_days", 10
        )
