...
class AttendanceCalculator:
    ...
    def calculate(
        self, records: List[AttendanceRecord], period: Optional[str] = None
    ) -> AttendanceSummary:
        """勤怠集計メイン処理
        ...
        # 出勤率の計算
        attendance_rate = (
            (attendance_days / business_days * 100) if business_days > 0 else 0.0
        )
        absence_rate = (
            (absence_days / business_days * 100) if business_days > 0 else 0.0
        )
        ...
        average_work_hours = (
            (total_work_minutes / 60 / attendance_days) if attendance_days > 0 else 0.0
        )
    ...
    def calculate_batch(
        self, records_by_employee: Dict[str, List[AttendanceRecord]]
    ...
    def _calculate_overtime_minutes(
    ...
    def _calculate_tardiness_and_early_leave(
        self, records: List[AttendanceRecord]
    ) -> Dict[str, int]:
        ...
        for record in records:
            ...
            if record.start_time and record.start_time > standard_start:
                ...
                standard_start_minutes = (
                    standard_start.hour * 60 + standard_start.minute
                )
    ...
    def _determine_period(
        self, records: List[AttendanceRecord], period: Optional[str]
    ) -> tuple[date, date]:
    ...
    def calculate_with_violations(
        self, records: List[AttendanceRecord], period: Optional[str] = None
    ) -> tuple[AttendanceSummary, List["WorkRuleViolation"]]:
    ...
    def _integrate_violation_warnings(
        self, summary: AttendanceSummary, violations: List["WorkRuleViolation"]
    ) -> AttendanceSummary:
