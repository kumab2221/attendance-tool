...
class TemplateManager:
    ...
    def _load_template_config(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if config_path.exists():
                ...
                logger.warning("テンプレート設定ファイルが見つかりません。デフォルト設定を使用します。")
    ...
    def generate_templated_report(
        ...
        try:
            # テンプレート設定の取得
            template_settings = self._get_template_settings(template_name, output_format)
            ...
            if output_format.lower() == "excel":
                result = self._generate_excel_with_template(
                    employee_summaries, department_summaries,
                    output_path, year, month, template_settings,
                    comparison_data, include_charts
                )
    ...
    def _get_template_settings(self, template_name: str, output_format: str) -> Dict[str, Any]:
        """テンプレート設定の取得"""
        template_settings = self.template_config.get("template_settings", {})
        
        if template_name not in template_settings:
            logger.warning(f"テンプレート '{template_name}' が見つかりません。デフォルトを使用します。")
    ...
    def _generate_comparison_data(
        ...
        for i in range(1, previous_months + 1):
            prev_date = datetime(current_year, current_month, 1) - timedelta(days=i*30)
    ...
    def _generate_excel_with_template(
        ...
        result = self.excel_exporter.export_excel_report(
            employee_summaries, department_summaries,
            output_path, year, month, include_charts
        )
        ...
        # テンプレート適用（ファイル後処理）
        try:
            self._apply_excel_template(result.file_path, template_settings, comparison_data)
            result.add_warning("テンプレート機能は基本実装です。詳細カスタマイゼーションは今後対応予定。")
        except Exception as e:
    ...
    def _add_excel_header_info(self, workbook, header_info: Dict[str, Any]) -> None:
        ...
        for sheet_name in workbook.sheetnames:
            ...
            # 生成日時
            worksheet["A4"] = f"Generated at: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
            
            # 既存データを下にシフト
            worksheet.insert_rows(1, 5)
    
    def _add_comparison_worksheet(self, workbook, comparison_data: Dict[str, Any]) -> None:
        ...
        comparison_sheet["A11"] = f"出勤率トレンド: {trends.get('attendance_trend', 'N/A')}"
        comparison_sheet["A12"] = f"残業時間トレンド: {trends.get('overtime_trend', 'N/A')}"
        comparison_sheet["A13"] = f"部門パフォーマンス: {trends.get('department_performance', 'N/A')}"
    ...
    def generate_multi_month_report(
        ...
        logger.info(f"複数月統合レポート生成開始: {start_year}-{start_month:02d} to {end_year}-{end_month:02d}")
        ...
        try:
            ...
            month_count = self._calculate_month_count(start_year, start_month, end_year, end_month)
            ...
            with open(file_path, 'w') as f:
                ...
                f.write(f"# 期間: {start_year}-{start_month:02d} から {end_year}-{end_month:02d}\n")
    ...
    def _calculate_month_count(self, start_year: int, start_month: int, end_year: int, end_month: int) -> int:
        """月数の計算"""
    ...
    def _aggregate_multi_month_data(self, data_by_month: Dict[str, Dict]) -> Dict[str, Any]:
        """複数月データの統合"""
        ...
        aggregated["summary_stats"] = {
            ...
            "average_monthly_employees": total_employees / len(data_by_month) if data_by_month else 0
        }
        ...
        try:
            ...
            if save_to_config:
                ...
                with open(config_path, 'w', encoding='utf-8') as f:
                    yaml.dump(self.template_config, f, default_flow_style=False, allow_unicode=True)
