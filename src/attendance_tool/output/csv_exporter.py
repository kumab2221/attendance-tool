...
class CSVExporter:
    ...
    def _build_export_config(self, config_dict: Dict[str, Any]) -> CSVExportConfig:
        ...
        for col_config in config_dict.get('columns', []):
            columns.append(CSVColumnConfig(
        ...
        return CSVExportConfig(
            filename_pattern=config_dict.get('filename_pattern', 'report_{year}_{month:02d}.csv'),
    ...
    def export_department_report(
        ...
        try:
            ...
            logger.info(f"部門別レポートを出力しました: {file_path} ({len(summaries)}件)")
            return result
