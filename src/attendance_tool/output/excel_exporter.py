...
class ExcelExporter:
    ...
    def _load_excel_config(self) -> None:
        ...
        try:
            ...
            logger.warning(f"Excel設定の読み込みに失敗しました。デフォルト設定を使用します: {e}")
    ...
    def export_excel_report(
        ...
        try:
            ...
            self.export_department_worksheet(workbook, department_summaries, year, month)
            self.export_summary_worksheet(workbook, employee_summaries, department_summaries, year, month, include_charts)
            ...
            if not employee_summaries and not department_summaries:
                result.add_warning("データが空のためヘッダーのみのファイルを作成しました")
            
            logger.info(f"Excelレポートを出力しました: {file_path} ({len(employee_summaries)}件)")
            return result
    ...
    def export_employee_worksheet(
        ...
        # ヘッダー行定義
        headers = [
            "社員ID", "氏名", "部署", "対象年月", "出勤日数", "欠勤日数",
            "遅刻回数", "早退回数", "総労働時間", "所定労働時間",
            "残業時間", "深夜労働時間", "有給取得日数"
        ]
    ...
    def export_department_worksheet(
        ...
        """部門別ワークシート出力"""
        worksheet = workbook.create_sheet(self.excel_config.worksheet_names["department"])
        
        # ヘッダー行定義
        headers = [
            "部署", "対象年月", "所属人数", "総出勤日数",
            "総欠勤日数", "総労働時間", "総残業時間", "平均出勤率"
        ]
    ...
    def export_summary_worksheet(
        ...
        total_work_days = sum(s.attendance_days for s in employee_summaries) if employee_summaries else 0
        avg_attendance_rate = sum(d.attendance_rate for d in department_summaries) / len(department_summaries) if department_summaries else 0
        total_overtime_hours = sum((s.scheduled_overtime_minutes + s.legal_overtime_minutes) / 60.0 for s in employee_summaries) if employee_summaries else 0
    ...
    def _apply_conditional_formatting(self, worksheet, summaries: List[DepartmentSummary]) -> None:
        """条件付き書式の適用"""
        if not summaries:
            return
        ...
        data_range = f"{attendance_rate_column}2:{attendance_rate_column}{len(summaries) + 1}"
    ...
    def _create_department_chart(self, worksheet, department_summaries: List[DepartmentSummary]) -> None:
        ...
        # データ範囲設定
        data = Reference(worksheet, min_col=chart_start_col + 1, min_row=1, max_row=len(department_summaries) + 1)
        categories = Reference(worksheet, min_col=chart_start_col, min_row=2, max_row=len(department_summaries) + 1)
    ...
    def _convert_employee_summary_to_row(self, summary: AttendanceSummary, year: int, month: int) -> Dict[str, Any]:
        ...
        overtime_hours = self._minutes_to_hours(summary.scheduled_overtime_minutes + summary.legal_overtime_minutes)
        ...
        return {
            ...
            "深夜労働時間": round(getattr(summary, 'late_night_work_minutes', 0) / 60.0, 2),
    ...
    def _convert_department_summary_to_row(self, summary: DepartmentSummary, year: int, month: int) -> Dict[str, Any]:
        ...
        # 推定値計算
        total_work_days = int(summary.employee_count * summary.average_work_minutes / 480) if summary.average_work_minutes > 0 else 0
