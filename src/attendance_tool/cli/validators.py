...
def validate_input_file(input_file):
    ...
    if not input_file.suffix.lower() == ".csv":
        raise ValidationError(
            f"入力ファイルはCSV形式である必要があります: {input_file}"
        )
        ...
        try:
            with open(input_file, "r", encoding="shift_jis") as f:
                ...
                if not first_line:
                    raise ValidationError(f"入力ファイルが空です: {input_file}")
            click.echo(
                "警告: ファイルのエンコーディングがShift_JISです。UTF-8を推奨します。"
            )
        except Exception:
            raise ValidationError(
                f"入力ファイルのエンコーディングが不正です: {input_file}"
            )
    except Exception as e:
        raise ValidationError(
            f"入力ファイルの読み込みに失敗しました: {input_file}, エラー: {e}"
        )
...
def validate_date_range(start_date, end_date):
    ...
    def parse_date(date_str, param_name):
        if not re.match(r"^\d{4}-\d{2}-\d{2}$", date_str):
            raise ValidationError(
                f"{param_name}の形式はYYYY-MM-DDである必要があります: {date_str}"
            )
    ...
    if start > end:
        raise ValidationError(
            f"開始日は終了日以前である必要があります: {start_date} > {end_date}"
        )
    ...
    max_date = (
        ...
        if today.month < 12
        else datetime(today.year + 1, 12, 31).date()
    )

    if start < min_date or end > max_date:
        raise ValidationError(
            f"日付は{min_date}から{max_date}の範囲である必要があります"
        )
    ...
    if output_path.suffix:
        ...
        if not parent_dir.exists():
            ...
            try:
                ...
                raise ValidationError(
                    f"出力ディレクトリの作成権限がありません: {parent_dir}"
                )
            except OSError as e:
                raise ValidationError(
                    f"出力ディレクトリの作成に失敗しました: {parent_dir}, エラー: {e}"
                )
...
def validate_formats(formats):
    ...
    for fmt in formats:
        if fmt not in valid_formats:
            raise ValidationError(
                f"不正な出力形式です: {fmt}. 有効な形式: {', '.join(valid_formats)}"
            )
...
def validate_report_types(report_types):
    ...
    for rtype in report_types:
        if rtype not in valid_types:
            raise ValidationError(
                f"不正なレポート種類です: {rtype}. 有効な種類: {', '.join(valid_types)}"
            )
...
def validate_chunk_size(chunk_size):
    ...
    if chunk_size < 1:
        raise ValidationError(
            f"チャンクサイズは1以上である必要があります: {chunk_size}"
        )

    if chunk_size > 100000:
        raise ValidationError(
            f"チャンクサイズは100000以下である必要があります: {chunk_size}"
        )
