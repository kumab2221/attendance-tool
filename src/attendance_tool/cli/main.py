...
@main.command()
@click.option('--input', '-i', 'input_file', required=True, type=click.Path(exists=True, path_type=Path),
              help='入力CSVファイルパス')
...
@click.option('--format', '-f', 'formats', multiple=True, 
              type=click.Choice(['csv', 'excel', 'both']), default=['csv'],
              help='出力形式')
@click.option('--report-type', '-r', 'report_types', multiple=True,
              type=click.Choice(['employee', 'department', 'daily', 'all']), default=['employee'],
              help='レポート種類')
@click.option('--config-dir', type=click.Path(exists=True, file_okay=False, path_type=Path),
              help='設定ディレクトリパス')
...
def process(input_file, output_path, month, start_date, end_date, year,
           formats, report_types, config_dir, verbose, quiet, dry_run,
           chunk_size, parallel):
    """勤怠データの処理・集計
