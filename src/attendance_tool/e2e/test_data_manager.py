...
class TestDataManager:
    ...
    def __init__(self):
        ...
        self.employee_names = [
            ...
            "松本理恵", "中村友香", "小林大輔", "加藤優子", "吉田直樹"
        ]
    
    def generate_standard_csv_data(self, output_file: Path, 
                                 num_employees: int = 3, 
                                 num_days: int = 3) -> None:
        """標準的なCSVテストデータを生成"""
        data = []
        ...
        for emp_id in range(1, num_employees + 1):
            emp_name = self.employee_names[emp_id - 1] if emp_id <= len(self.employee_names) else f"従業員{emp_id}"
    ...
    def generate_large_dataset(self, output_file: Path,
    ...
    def generate_corrupted_csv(self, output_file: Path) -> None:
        ...
        with open(output_file, 'w', encoding='utf-8') as f:
            ...
            f.write("EMP001,テスト太郎,テスト部,2024-02-30,25:00,26:00,01:00\n")  # 不正な日付と時刻
            f.write("EMP002,,テスト部,2024-01-01,09:00,18:00,\n")  # 名前と休憩時間が欠損
            f.write("EMP003,テスト花子,,2024-01-01,09:00,07:00,01:00\n")  # 部門欠損、退勤＜出勤
    
    def create_expected_output(self, input_data: List[Dict[str, Any]], 
