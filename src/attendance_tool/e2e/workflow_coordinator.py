...
class E2EWorkflowCoordinator:
    ...
    def _get_column_mapping(self, raw_data: pd.DataFrame) -> Dict[str, Optional[str]]:
        """カラムマッピングを取得"""
        mapping = {"emp_id": None, "emp_name": None, "dept": None}
    ...
    def _create_employee_summary(
        self, emp_data: pd.DataFrame, column_mapping: Dict[str, Optional[str]]
    ) -> Dict[str, Any]:
        """従業員サマリーを作成"""
        return {
            "社員ID": emp_data.iloc[0][column_mapping["emp_id"]],
            "氏名": (
                emp_data.iloc[0][column_mapping["emp_name"]]
                if column_mapping["emp_name"]
                else "N/A"
            ),
            "部署": (
                emp_data.iloc[0][column_mapping["dept"]]
                if column_mapping["dept"]
                else "N/A"
            ),
            "出勤日数": len(emp_data),
    ...
    def _export_report(
        self, summary_data: List[Dict[str, Any]], output_dir: Path, month: str
    ) -> Path:
        """レポートを出力"""
    ...
    def _create_success_result(
        self, records_count: int, output_file: Path, validation_result: Any
    ) -> Dict[str, Any]:
        """成功結果を作成"""
        return {
            "status": "success",
    ...
    def execute_cli_workflow(
        ...
        result = runner.invoke(
            main,
            ["--input", str(input_file), "--output", str(output_dir), "--month", month],
        )
