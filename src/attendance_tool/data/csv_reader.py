...
class CSVReader:
    ...
    def _load_config(self) -> Dict:
        ...
        if not self.config_path:
            # デフォルト設定パス
            default_config = Path(__file__).parent.parent.parent.parent / "config" / "csv_format.yaml"
    ...
    def load_file(self, file_path: str, encoding: Optional[str] = None) -> pd.DataFrame:
        ...
        try:
            ...
            if validation_result.has_critical_errors():
                critical_errors = [e.message for e in validation_result.errors if "必須" in e.message or "不存在" in e.message]
    ...
    def _validate_time_fields(self, row: pd.Series, row_idx: int, 
                             column_mapping: Dict[str, str], 
                             errors: List[ValidationErrorDetail],
    ...
    def _validate_work_hours(self, row: pd.Series, row_idx: int,
                           column_mapping: Dict[str, str],
                           warnings: List[ValidationWarning]) -> None:
        """勤務時間を検証"""
        if "start_time" in column_mapping and "end_time" in column_mapping:
