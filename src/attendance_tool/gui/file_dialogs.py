...
class FileDialogs:
        ...
        if file_path:
            ...
            if self.validate_csv_file(file_path):
                ...
                messagebox.showerror(
                    "エラー", 
                    "選択されたファイルは有効なCSVファイルではありません。"
                )
                return None
        
        return None
    
    def select_output_directory(self, initial_dir: Optional[str] = None) -> Optional[str]:
        """出力ディレクトリ選択ダイアログを表示
    ...
    def save_csv_file(self, initial_dir: Optional[str] = None, 
                      initial_filename: str = "attendance_report.csv") -> Optional[str]:
        """CSV保存ダイアログを表示
