...
class MainWindow:
    ...
    def _create_file_selection_frame(self, parent):
        ...
        # 入力ファイル
        ttk.Label(frame, text="入力CSVファイル:").grid(
        ...
        ttk.Button(frame, text="参照", command=self.select_input_file).grid(
            row=0, column=2, padx=5, pady=2
        )

        # 出力ディレクトリ
        ttk.Label(frame, text="出力ディレクトリ:").grid(
            row=1, column=0, sticky=tk.W, pady=2
        )
        self.output_entry = ttk.Entry(
            frame, textvariable=self.output_dir_path, width=60
        )
        self.output_entry.grid(row=1, column=1, sticky=(tk.W, tk.E), padx=5, pady=2)
        ttk.Button(frame, text="参照", command=self.select_output_directory).grid(
    ...
    def _create_log_frame(self, parent):
        ...
        # ログクリアボタン
        ttk.Button(frame, text="ログクリア", command=self._clear_log).grid(
    ...
    def _run_attendance_processing(
        self, input_file: str, output_dir: str, target_month: str
    ):
        """勤怠集計処理の実行（簡易版）"""
        ...
        if result["status"] == "success":
            ...
            self.add_log(
                f"処理中にエラーが発生しました: {result['error_message']}", "error"
            )
