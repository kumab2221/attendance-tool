...
class SettingsWindow:
    ...
    def _create_general_tab(self, notebook):
        ...
        ttk.Button(entry_frame, text="参照", command=self._browse_input_dir).pack(
        ...
        ttk.Button(entry_frame2, text="参照", command=self._browse_output_dir).pack(
            side=tk.RIGHT, padx=(5, 0)
        )
        ...
        ttk.Checkbutton(
            frame, text="出力時に自動バックアップを作成", variable=self.auto_backup
    ...
    def _create_output_tab(self, notebook):
        ...
        ttk.Checkbutton(
            frame, text="ファイル名にタイムスタンプを追加", variable=self.add_timestamp
        ).grid(row=2, column=0, columnspan=2, sticky=tk.W, pady=10)
    ...
    def _create_button_frame(self):
        ...
        ttk.Button(
            button_frame, text="デフォルトに戻す", command=self._reset_to_default
    ...
    def _load_current_settings(self):
        ...
        self.filename_template.set(
            self.settings.get("filename_template", "attendance_report_{YYYY}_{MM}")
        )
