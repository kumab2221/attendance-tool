...
class EnhancedCSVReader(CSVReader):
    ...
    def load_and_validate(
        ...
        try:
            ...
            if len(critical_errors) > 0:
                ...
                self.logger.warning(
                    f"重大エラーが {len(critical_errors)} 件検出されました"
                )
    ...
    def get_enhanced_validation_result(
        self, file_path: str, encoding: Optional[str] = None
    ) -> dict:
        """拡張検証結果取得
        ...
        try:
            # 完全ワークフロー実行
            cleaned_df, validation_report, cleaning_result = (
                self.load_validate_and_clean(file_path, encoding)
            )
    ...
    def export_comprehensive_report(
        self, file_path: str, output_dir: str, encoding: Optional[str] = None
    ) -> dict:
        """包括的レポート出力
        ...
        try:
            ...
            validation_report_path = os.path.join(
                output_dir, f"{base_name}_validation_report_{timestamp}.csv"
            )
            ...
            # 2. 清洗ログ出力
            cleaning_log_path = os.path.join(
                output_dir, f"{base_name}_cleaning_log_{timestamp}.csv"
            )
            result["reports"]["cleaning_result"].export_corrections_log(
                cleaning_log_path
            )
            ...
            cleaned_data_path = os.path.join(
                output_dir, f"{base_name}_cleaned_{timestamp}.csv"
            )
            result["data"]["cleaned_dataframe"].to_csv(
                cleaned_data_path, index=False, encoding="utf-8-sig"
            )
            output_files["cleaned_data"] = cleaned_data_path

            # 4. サマリーレポート作成
            summary_path = os.path.join(
                output_dir, f"{base_name}_summary_{timestamp}.txt"
            )
    ...
    def _create_summary_report(self, result: dict, output_path: str) -> None:
        ...
        with open(output_path, "w", encoding="utf-8") as f:
            ...
            f.write(
                f"文字エンコーディング: {result['file_info'].get('encoding', '自動検出')}\n"
            )
