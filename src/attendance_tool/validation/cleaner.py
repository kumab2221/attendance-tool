...
class DataCleaner:
    ...
    def _setup_default_mappings(self):
        """デフォルトマッピング設定"""
        # 部署名マッピング
        self.department_mapping = self.config.get('department_mapping', {
            '開発課': '開発部',
            '開発チーム': '開発部',
            'Development': '開発部',
            '営業課': '営業部',
        ...
        # クレンジングレベル
        self.cleaning_level = self.config.get('level', 'standard')  # conservative, standard, aggressive
    ...
    def suggest_corrections(self, errors: List[ValidationError], context: Dict = None) -> List[CorrectionSuggestion]:
        """修正提案生成
        ...
        for error in errors:
            if '時刻論理' in error.message or '出勤時刻が退勤時刻より遅い' in error.message:
                # 時刻論理エラーの修正提案
    ...
    def _clean_employee_names(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if 'employee_name' in df.columns:
            df['employee_name'] = df['employee_name'].apply(self._normalize_employee_name)
        return df
    ...
    def _normalize_employee_name(self, name: Any) -> str:
        ...
        if self.cleaning_level == 'aggressive':
            ...
            if re.match(r'^[A-Z\s]+$', name_str):
                ...
                name_str = alphabet_to_japanese.get(name_str, name_str)
    ...
    def _suggest_time_correction(self, error: ValidationError, context: Dict) -> Optional[CorrectionSuggestion]:
        """時刻修正提案"""
    ...
    def _suggest_date_correction(self, error: ValidationError, context: Dict) -> Optional[CorrectionSuggestion]:
        """日付修正提案"""
    ...
    def _suggest_employee_id_correction(self, error: ValidationError, context: Dict) -> Optional[CorrectionSuggestion]:
        """社員ID修正提案"""
    ...
    def _suggest_department_correction(self, error: ValidationError, context: Dict) -> Optional[CorrectionSuggestion]:
        """部署名修正提案"""
        original_dept = str(error.value)
        candidates = self.config.get('department_candidates', ['開発部', '営業部', '総務部', 'システム部'])
