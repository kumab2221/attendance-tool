...
class TestErrorHandlingIntegration:
    ...
    def test_req_104_work_hours_integration(self):
        ...
        df = pd.DataFrame({
            ...
            'end_time': ['09:00', '08:30', '07:00'],  # 25時間勤務、負時間、25時間勤務
        ...
        work_hour_errors = [
            error for error in report.errors
            if ("勤務時間" in error.message or "24時間" in error.message or 
                "負の" in error.message)
        ]
    ...
    def test_edge_204_future_date_integration(self):
        ...
        validator = DataValidator(config={
            ...
            'future_date_warning_days': 7
        }, rules=[])
        report = validator.validate_dataframe(df)
        
        # Then: 未来日警告・エラー検出
        future_date_issues = [
            issue for issue in (report.errors + report.warnings)
        ...
        date_suggestions = [
            sugg for sugg in suggestions
            if sugg.correction_type == 'date_year'
        ]
        if date_suggestions:
    ...
    def test_multiple_validation_rules_integration(self):
        ...
        df = pd.DataFrame({
            'employee_id': ['', 'EMP002', 'INVALID_ID'],  # 空、正常、フォーマットエラー
            'employee_name': ['田中太郎', '', '佐藤次郎'],    # 正常、空、正常
            'work_date': ['2024-01-15', '2025-12-31', 'invalid'],  # 正常、未来、無効
...
class TestPerformanceIntegration:
    ...
    def create_large_test_csv(self, size: int):
        ...
        with open(csv_path, 'w', encoding='utf-8') as f:
            f.write('employee_id,employee_name,work_date,start_time,end_time,department\n')
            for i in range(size):
                # 意図的に一部エラーを含む
                ...
                end_time = '18:00'
                dept = '開発部' if i % 3 == 0 else ('営業部' if i % 3 == 1 else '総務部')
