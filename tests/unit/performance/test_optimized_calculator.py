...
from attendance_tool.performance.models import PerformanceConfig, PerformanceReport, PerformanceMetrics
...
class TestPerformanceOptimizedCalculator:
    ...
    def test_calculate_batch_optimized_parallel(self, sample_employee_records, performance_config):
        """最適化バッチ計算（並列）テスト"""
        # 期待結果: 並列処理による高速化
        ...
        # 性能目標の確認
        assert processing_time <= 300, f"Processing time {processing_time:.2f}s exceeds 5 minute limit"
    ...
    def test_calculate_batch_optimized_sequential(self, sample_employee_records, performance_config):
        """最適化バッチ計算（逐次）テスト"""
        # 期待結果: 小規模データでの逐次処理選択
        
        # 小規模データセット（5名分）
        small_dataset = {k: v for i, (k, v) in enumerate(sample_employee_records.items()) if i < 5}
        ...
        results = calculator.calculate_batch_optimized(
            small_dataset,
            parallel=True  # 小規模なので自動的に逐次処理が選択される
        )
        end_time = time.time()
        ...
        # 処理モード選択の確認
        assert calculator.last_processing_mode == "sequential", "Small dataset should use sequential processing"
        
        # 処理時間の妥当性確認
        processing_time = end_time - start_time
        assert processing_time < 10, f"Sequential processing time {processing_time:.2f}s too slow"
    ...
    def test_calculate_with_chunking(self, performance_config):
        ...
        assert memory_variance <= 200, f"Memory variance {memory_variance}MB too high for chunking"
        
        # 処理時間の妥当性
        processing_time = end_time - start_time
        assert processing_time < 120, f"Chunking processing time {processing_time:.2f}s exceeds 2 minutes"
    
    def test_performance_monitoring_integration(self, sample_employee_records, performance_config):
        """性能監視統合テスト"""
        # 期待結果: リアルタイム性能監視
    ...
    def test_memory_cleanup_and_gc_optimization(self, sample_employee_records, performance_config):
        """メモリクリーンアップとGC最適化テスト"""
        # 期待結果: 効率的なメモリ管理
        ...
        for iteration in range(5):
            ...
            assert memory_increase <= 100, f"Memory leak detected: {memory_increase}MB increase in iteration {iteration}"
        
        # 最終GC統計
        final_gc_stats = gc.get_stats()
        
        # GC最適化の効果確認
        gc_optimization_effect = calculator.performance_monitor.get_gc_optimization_stats()
    ...
    def test_performance_report_generation(self, sample_employee_records, performance_config):
        """性能レポート生成テスト"""
        # 期待結果: 詳細な性能レポート作成
    ...
    def test_configuration_impact_on_performance(self, sample_employee_records):
        ...
        start_time = time.time()
        basic_results = basic_calculator.calculate_batch_optimized(sample_employee_records)
        ...
        start_time = time.time()
        optimized_results = optimized_calculator.calculate_batch_optimized(sample_employee_records)
        ...
        memory_efficiency = basic_memory / optimized_memory if optimized_memory > 0 else 1
        
        # 最適化効果の確認（20%以上の改善）
        assert time_improvement >= 0.2 or memory_efficiency >= 1.2, \
    ...
    def test_scalability_performance(self):
        ...
        for i in range(1, len(processing_times)):
            ...
            assert scaling_efficiency <= 0.5, f"Poor scaling efficiency: {scaling_efficiency:.1%} for size ratio {size_ratio:.1f}"
    ...
    def test_stress_test_maximum_load(self):
        ...
        processing_time = end_time - start_time
        assert processing_time <= 1200, f"Stress test processing time {processing_time:.1f}s exceeds 20 minutes"
