...
class TestAdaptiveChunking:
    ...
    def test_optimal_chunk_size_calculation(self, large_dataset):
        ...
        chunker = AdaptiveChunking(
            initial_chunk_size=10000, memory_limit=512 * 1024 * 1024  # 512MB
        )
        ...
        for available_memory_mb, expected_min_chunk in memory_scenarios:
            with patch("psutil.virtual_memory") as mock_memory:
                ...
                assert (
                    optimal_size >= expected_min_chunk
                ), f"Chunk size {optimal_size} below minimum {expected_min_chunk}"
                assert (
                    optimal_size <= chunker.max_chunk_size
                ), f"Chunk size {optimal_size} exceeds maximum"

                # チャンクサイズが妥当な範囲内にあることを確認
                assert (
                    1000 <= optimal_size <= 100000
                ), f"Chunk size {optimal_size} outside reasonable range"
    ...
    def test_memory_based_adaptation(self, large_dataset):
        ...
        chunker = AdaptiveChunking(
            memory_limit=256 * 1024 * 1024, adaptive_sizing=True  # 256MB制限
        )
        ...
        # メモリ制限違反率0%の確認
        violation_rate = (
            memory_violations / processed_chunks if processed_chunks > 0 else 0
        )
        assert (
            violation_rate == 0
        ), f"Memory violation rate {violation_rate:.2%} exceeds 0%"
    ...
    def test_chunk_dependency_management(self):
        ...
        for employee_id in range(100):
            for day in range(31):
                ...
                prev_overtime = (
                    0 if day == 0 else time_series_data[-1]["overtime_hours"]
                )
        ...
        chunker = AdaptiveChunking(
            dependency_aware=True, dependency_columns=["employee_id", "date"]
        )
        ...
        # データ整合性100%維持の確認
        violation_rate = (
            dependency_violations / len(chunk_boundaries) if chunk_boundaries else 0
        )
        assert (
            violation_rate == 0
        ), f"Dependency violation rate {violation_rate:.2%} exceeds 0%"
        ...
        for i, (start, end) in enumerate(chunk_boundaries):
            if i > 0:
                ...
                assert (
                    start == prev_end
                ), f"Gap detected between chunks: {prev_end} -> {start}"
    ...
    def test_dynamic_size_adjustment(self, large_dataset):
        ...
        for i, chunk in enumerate(
            chunker.process_with_adaptive_chunking(large_dataset)
        ):
            ...
            if i > 2:  # 最初の3チャンク後から効率測定
                recent_efficiency = chunker.calculate_processing_efficiency(
                    processing_times[-3:]
                )
        ...
        if len(processing_times) >= 10:
            ...
            ), f"Efficiency improvement {efficiency_improvement:.2%} below 10%"
    ...
    def test_partial_failure_recovery(self, large_dataset):
        ...
        chunker = AdaptiveChunking(
            error_recovery=True, retry_failed_chunks=True, max_retries=3
        )
        ...
        if recovery_times:
            ...
            ), f"Average recovery time {avg_recovery_time:.2f}s exceeds 10s"
            assert (
                max_recovery_time < 15
            ), f"Max recovery time {max_recovery_time:.2f}s exceeds 15s"
    ...
    def test_memory_pressure_adaptation(self):
        ...
        chunker = AdaptiveChunking(
            initial_chunk_size=20000, memory_pressure_threshold=0.85
        )
        ...
        for memory_usage_percent, expected_max_size in memory_scenarios:
            with patch("psutil.virtual_memory") as mock_memory:
                ...
                assert (
                    adjusted_size <= expected_max_size
                ), f"Size {adjusted_size} not adapted for {memory_usage_percent:.0%} memory usage"
                assert (
                    adjusted_size >= chunker.min_chunk_size
    ...
    def test_chunk_processing_benchmark(self, large_dataset):
        ...
        ), f"Processing speed {records_per_second:.1f} records/sec below 5000 threshold"

        chunks_per_second = chunks_processed / processing_time
        assert (
            chunks_per_second >= 10
        ), f"Chunk processing speed {chunks_per_second:.1f} chunks/sec below 10 threshold"

        # メモリ効率の確認
        peak_memory_mb = chunker.get_peak_memory_usage_mb()
        memory_per_record = peak_memory_mb / total_records
        assert (
            memory_per_record < 0.01
        ), f"Memory per record {memory_per_record:.4f}MB too high"
...
class TestOptimizedCSVProcessor:
    ...
    def test_large_csv_chunked_reading(self, large_csv_file):
        ...
        processor = OptimizedCSVProcessor(chunk_size=10000)
    ...
    def test_parallel_csv_processing(self, tmp_path):
        ...
        assert (
            actual_speedup >= 3.0
        ), f"Parallel speedup {actual_speedup:.2f}x below 3.0x target"
    ...
    def test_streaming_csv_validation(self, large_csv_file):
        ...
        start_time = time.time()
        no_validation_chunks = list(
            processor_no_validation.read_csv_in_chunks(large_csv_file)
        )
        ...
        for chunk_result in processor.read_csv_in_chunks_with_validation(
            large_csv_file
        ):
        ...
        if validation_errors:
            ...
            assert (
                error_rate <= 0.05
            ), f"Validation error rate {error_rate:.1%} exceeds 5%"
    ...
    def test_encoding_detection_optimization(self, tmp_path):
        ...
        processor = OptimizedCSVProcessor(
            auto_detect_encoding=True, encoding_cache=True
        )
        ...
        for file_path, expected_encoding in test_files:
            ...
            ), f"Encoding detection failed: expected {expected_encoding}, got {detected_encoding}"
        ...
        assert (
            time_per_gb < 1.0
        ), f"Encoding detection time {time_per_gb:.2f}s/GB exceeds 1 second/GB"
        ...
        if processor.encoding_cache_enabled:
            ...
            assert (
                cache_speedup >= 10
            ), f"Cache speedup {cache_speedup:.1f}x below 10x target"
