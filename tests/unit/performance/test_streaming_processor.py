...
class TestStreamingProcessor:
    ...
    def test_stream_chunk_processing(self, sample_records):
        ...
        # チャンク処理とメモリ監視
        for chunk_result in processor.process_stream(
            sample_records, chunk_size=chunk_size
        ):
    ...
    def test_backpressure_control(self):
        ...
        processor = StreamingProcessor(
            memory_limit=512 * 1024 * 1024, backpressure_threshold=0.8
        )
    ...
    def test_stream_aggregation(self, sample_records):
        ...
        assert (
            accuracy < 0.01
        ), f"Aggregation accuracy {accuracy:.3f} exceeds 1% tolerance"
    ...
    def test_progress_tracking(self, sample_records):
        ...
        if len(progress_updates) > 1:
            ...
            assert (
                max_interval <= 1.0
            ), f"Progress update interval {max_interval}s exceeds 1 second"
    ...
    def test_memory_efficient_streaming(self):
        ...
        for i, summary in enumerate(
            processor.process_stream(large_data_generator(), chunk_size=1000)
        ):
    ...
    def test_error_handling_resilience(self, sample_records):
        ...
        for result in processor.process_stream_with_error_handling(
            error_prone_generator()
        ):
            if result.is_error:
                error_count += 1
            else:
        ...
        assert (
            success_rate >= 0.9
        ), f"Success rate {success_rate:.2f} below 90% threshold"
    ...
    def test_streaming_performance_benchmark(self, sample_records):
        ...
        ), f"Processing speed {records_per_second:.1f} records/sec below 1000 threshold"

        # 全体的な処理時間の確認（3100レコードを5秒以内）
        assert (
            processing_time <= 5.0
        ), f"Processing time {processing_time:.2f}s exceeds 5 second limit"

        # メモリ効率の確認
        peak_memory = processor.get_peak_memory_usage_mb()
        memory_per_record = peak_memory / len(sample_records)
        assert (
            memory_per_record < 0.1
        ), f"Memory per record {memory_per_record:.3f}MB too high"
