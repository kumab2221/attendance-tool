...
try:
    from attendance_tool.validation.cleaner import (
        CleaningResult,
...
class TestDataCleanerAutoCorrection:
    ...
    def test_clean_employee_name_formatting(self):
        ...
        cleaner = DataCleaner(
            config={
                "name_cleaning": {"trim_whitespace": True, "normalize_alphabet": True}
            }
        )
        cleaned_df = cleaner.apply_auto_corrections(df)
        ...
        assert (
            cleaned_df.loc[1, "employee_name"] == "山田花子"
        )  # アルファベット→ひらがな変換
...
class TestDataCleanerSuggestions:
    ...
    def test_suggest_time_corrections(self):
        ...
        if (
            DataCleaner is None
            or ValidationError is None
            or CorrectionSuggestion is None
        ):
    ...
    def test_suggest_employee_id_corrections(self):
        ...
        # When: 修正提案生成（社員名から推測）
        record_context = {"employee_id": "EMP1", "employee_name": "田中太郎"}
        cleaner = DataCleaner(config={})
        ...
        assert (
            "EMP001" in suggestion.suggested_value
            or "EMP0001" in suggestion.suggested_value
        )
...
class TestDataCleanerCleaningResult:
    ...
    def test_cleaning_result_creation(self):
        ...
        original_df = pd.DataFrame(
            {"employee_id": ["EMP001", "EMP002"], "start_time": ["9:00", "09:00:00"]}
        )

        cleaned_df = pd.DataFrame(
            {"employee_id": ["EMP001", "EMP002"], "start_time": ["09:00", "09:00"]}
        )
...
class TestDataCleanerIntegration:
    ...
    def test_cleaning_configuration_levels(self):
        ...
        assert (
            conservative_result.loc[1, "employee_name"] == "YAMADA Hanako"
        )  # アルファベットはそのまま

        # Aggressive: 名前の変換も実行
        assert aggressive_result.loc[0, "employee_name"] == "田中太郎"
        assert (
            aggressive_result.loc[1, "employee_name"] == "山田花子"
        )  # アルファベット変換実行
