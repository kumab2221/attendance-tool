...
class TestErrorLogger(unittest.TestCase):
    ...
    def test_log_level_output_routing(self):
        ...
        test_cases = [
            ...
            {"level": "ERROR", "expected_outputs": ["console", "file"]},
            {"level": "WARNING", "expected_outputs": ["file"]},
            {"level": "INFO", "expected_outputs": ["file"]},  # デバッグモード時のみ
        ]
        ...
        for case in test_cases:
            with self.subTest(case=case):
                ...
                outputs = self.logger.log_with_level(
                    message="Test message", level=case["level"]
                )
