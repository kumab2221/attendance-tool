...
class TestFileValidation:
    ...
    def test_validate_input_file_not_exists(self, runner):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            assert any(keyword in error_msg for keyword in ['does not exist', 'not found', '存在しません', '見つかりません'])
...
class TestErrorHandling:
    ...
    def test_command_not_found(self, runner):
        ...
        assert result.exit_code != 0
        assert 'no such command' in result.output.lower() or 'command not found' in result.output.lower()
