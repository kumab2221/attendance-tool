...
def run_lizard(
    source_dirs, threshold=10, output_file=None, output_format="text", verbose=False
):
...
def generate_summary_report(stdout, output_dir):
    ...
    if complexity_values:
        ...
        stats["average_complexity"] = round(
            sum(complexity_values) / len(complexity_values), 2
        )
    ...
    with open(summary_file, "w", encoding="utf-8") as f:
        json.dump(
            {"stats": stats, "high_complexity_functions": high_complexity_functions},
            f,
            indent=2,
            ensure_ascii=False,
        )
...
使用例:
    ...
    parser.add_argument("--output", "-o", type=str, help="レポート出力ファイル")
